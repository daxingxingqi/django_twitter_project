...
class CommentViewSet(viewsets.GenericViewSet):
    """
    尽量不要继承自model viewset，下面就是默认方法
    只实现 list(GET), create（POST）, update(PUT /api/comments/1/), destroy 的方法
    ...
    def destroy(self, request, *args, **kwargs):
        comment = self.get_object() # 会调用queryset = Comment.objects.all() 然后filter出当前用户的comment
        comment.delete()
